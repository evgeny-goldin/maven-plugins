...
class IvyArtifactResolver implements ArtifactResolver {
     ...
     */
    @Requires({ request && request.artifact.groupId.startsWith( GMojoUtils.IVY_PREFIX ) })
    @Ensures({ result })
    ArtifactResult resolveIvy( ArtifactRequest request )
    {
        final a            = request.artifact
        final organisation = a.groupId.substring( GMojoUtils.IVY_PREFIX.size())
    ...
    {
        request.artifact.groupId.startsWith( GMojoUtils.IVY_PREFIX ) ?
            resolveIvy( request ) :
